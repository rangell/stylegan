...
def E_basic(
    ...
    labels_in,                          # Second input: Labels [minibatch, label_size].
    num_channels        = 1,            # Number of input color channels. Overridden based on dataset.
    latent_size        = 512,          # Disentangled latent (W) dimensionality.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def block(x, res): # res = 2..resolution_log2
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res >= 3: # 8x8 and up
                ...
                with tf.variable_scope('Dense0'):
                    x = act(apply_bias(dense(x, fmaps=nf(res-2), gain=gain, use_wscale=use_wscale)))
                with tf.variable_scope('mu_out'):
                    mu = act(apply_bias(dense(x, fmaps=nf(res-2), gain=gain, use_wscale=use_wscale)))
                with tf.variable_scope('log_var_out'):
                    log_var = act(apply_bias(dense(x, fmaps=nf(res-2), gain=gain, use_wscale=use_wscale)))
    ...
    if structure == 'recursive':
        ...
        def grow(res, lod):
            ...
            if res > 2: y = cset(y, (lod_in > lod), lambda: tflib.lerp(x, fromrgb(downscale2d(images_in, 2**(lod+1)), res - 1), lod_in - lod))
            return y()
        mu, log_var = grow(2, resolution_log2 - 2)

    assert mu.dtype == tf.as_dtype(dtype)
    assert log_var.dtype == tf.as_dtype(dtype)
