...
def training_loop(
    ...
    E_opt_args              = {},       # Options for encoder optimizer.

    G_loss_args             = {},       # Options for generator loss.
    D_loss_args             = {},       # Options for discriminator loss.
    E_loss_args             = {},       # Options for encoder loss.
    ...
    print('made it to the training_loop')
    ...
    for gpu in range(submit_config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('E_loss'), tf.control_dependencies(lod_assign_ops):
                E_loss = dnnlib.util.call_func_by_name(E=E_gpu, Gs_pre=Gs_pre, opt=E_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals, labels=labels,  **E_loss_args)
